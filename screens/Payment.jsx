...
// -------------------------------
const logos = {
  kpay: require("../assets/images/kpay.png"),
  wavepay: require("../assets/images/wavepay.png"),
  aya: require("../assets/images/aya.png"),
  cbpay: require("../assets/images/cbpay.jpeg"),
  uab: require("../assets/images/uab.png"),
};
...
const PaymentBottomSheet = forwardRef((props, externalRef) => {
  ...
  const pickScreenshot = async () => {
    try {
            ...
            setImage(manipulated.base64 ? { uri: manipulated.uri, base64: manipulated.base64 } : null);
...
const amount = parseFloat(text.amount_ks.replace(/[,]/g, ''));
...
async function registerPaymentToBackend(payload) {
  ...
  const sendToServer = async () => {
    if (!image) {
      Alert.alert("Missing", "Please upload screenshot first.");
      return;
    }
    setUploading(true);
    try {
      ...
      const cleanOcr = ocrRes.text.replace(/`/g, "").replace(/\n/g, " ").trim();
      const jsonocr = JSON.parse(cleanOcr);
      setOcrResult(jsonocr);
      console.log("OCR Result:", ocrResult.amount_ks);
      const validation = await validateOCRText(ocrResult);
      console.log("Validation Result:", validation);

       const { id } = getDeviceInfo()
      ...
      //       // If OK, register payment
            const payload = {
              ...
              transactionId: ocrResult.transaction_no || null,
              amount: ocrResult.amount_ks || null,
              status: 'PendingReview',
              transactionDateTime: ocrResult.transaction_time.replace(" ", "T") || null
            };      
  ...
  `}
            ...
            {/* Send Screenshot */}
            <Pressable
              disabled={ocrResult || uploading ? true : false}
